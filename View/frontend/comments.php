...
<div class="fixed-action-btn">
    <a class="btn-floating modal-trigger btn-large cyan" href="#addAComment"><i class="large material-icons">mode_edit</i></a>
</div>
...
?>
        ...
        <div class="card cyan darken-2" id="comm_cards">
            <div class="card-content">
                <span class="card-title"> <strong> <?= $response->note ?>/5 - <?= $response->comment_title ?> </strong> </span>
                <p>Publié par <?= $response->pseudo ?> le <?= date("d/m/Y", strtotime($response->date)) ?></p>
                <p><?= nl2br($response->comment) ?></p>
                <a href="index.php?url=warning&id=<?= $response->id ?>" class="white-text right">Signaler ce commentaire</a>
            </div>
        </div>
